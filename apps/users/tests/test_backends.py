...
TEST_PASSWORD = "test-login-123"  # pragma: allowlist secret
...
class TestEmailOrUsernameModelBackend:
    def test_authenticate_with_username_success(self):
        ...
        result = backend.authenticate(request=None, username="testuser", password=TEST_PASSWORD)
    ...
    def test_authenticate_returns_none_if_username_is_none_even_with_kwargs(self):
        backend = EmailOrUsernameModelBackend()
        result = backend.authenticate(request=None, username=None, password=TEST_PASSWORD)
        assert result is None
    ...
    def test_authenticate_returns_none_if_wrong_password(self):
        User.objects.create_user(
            username="wrongpass",
        ...
        result = backend.authenticate(
            request=None,
            username="wrongpass",
            password="bad-password-123",  # pragma: allowlist secret
        )
        assert result is None
