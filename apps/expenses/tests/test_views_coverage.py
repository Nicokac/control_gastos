...
@pytest.mark.django_db
class TestExpenseListViewAmountFilter:
    """Líneas 118-126: filtros amount_min y amount_max."""

    def test_filter_amount_min(self, authenticated_client, user, expense_category, expense_factory):
        expense_factory(
            user,
            expense_category,
            description="Barato",
        ...
        url = reverse("expenses:list")
        response = authenticated_client.get(url, {"amount_min": "1000"})

        assert response.status_code == 200
        content = response.content.decode()
        assert "Caro" in content
        assert "Barato" not in content

    def test_filter_amount_max(self, authenticated_client, user, expense_category, expense_factory):
        expense_factory(
            user,
            expense_category,
            description="Barato",
        ...
        assert response.status_code == 200
        content = response.content.decode()
        assert "Barato" in content
        assert "Caro" not in content

    def test_filter_amount_min_and_max_combined(
        self, authenticated_client, user, expense_category, expense_factory
    ):
        expense_factory(
            user,
        ...
        url = reverse("expenses:list")
        response = authenticated_client.get(url, {"amount_min": "100", "amount_max": "1000"})
