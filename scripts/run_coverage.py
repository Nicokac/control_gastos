...
def main():
    ...
    parser.add_argument(
        "--html", action="store_true", help="Generar reporte HTML y abrirlo en navegador"
    )
    parser.add_argument("--xml", action="store_true", help="Generar reporte XML (para CI/CD)")
    parser.add_argument(
        "--fail-under", type=int, default=80, help="Umbral mínimo de cobertura (default: 80)"
    )
    parser.add_argument("--verbose", "-v", action="store_true", help="Mostrar output detallado")
    parser.add_argument("pytest_args", nargs="*", help="Argumentos adicionales para pytest")
